...
export async function imgurDelete(imageHash: string): Promise<boolean> {
    try {
        return await fetch(`https://api.imgur.com/3/image/${imageHash}`, {
            method: 'DELETE',
            headers: {
                Authorization: `Client-ID ${credentials.ImgurClientID}`,
            },
...
// console.log(await imgurDelete('d7sbIt1fdSEzGVA'));
